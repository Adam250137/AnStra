...
extern double station_latitude, station_longitude, station_height;
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
	...
	if(strstr((char*)Buf, "TOGGLE") != NULL){HAL_GPIO_TogglePin(LED_PIN_GPIO_Port, LED_PIN_Pin);}

	else if(strstr((char*)Buf, "SET_ANGLES ") != NULL){
		num = strtok ((char*)Buf," ");
		if(num!=NULL){num = strtok(NULL, " "); s1=atoi(num);}
		if(num!=NULL){num = strtok(NULL, " "); s2=atoi(num);}
	}
		...
		sprintf((char*)TxData, "SEND_POSITION latitude %f longitude %f height %f", station_latitude,
